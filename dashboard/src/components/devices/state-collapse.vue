  ...
  <n-collapse v-model:expanded-names="expanded">
    <n-collapse-item title="Reported State" name="reported">
      <template #header-extra v-if="reported">
        <n-button tertiary circle type="info"
          @click.stop.prevent="handleCopy(JSON.stringify(reported.data, null, 2))">
          <template #icon>
    ...
    <n-collapse-item title="Desired State" name="desired">
      <template #header-extra v-if="desired">
        <n-button tertiary circle type="info"
          @click.stop.prevent="handleCopy(JSON.stringify(desired.data, null, 2))">
          <template #icon>
            <n-icon><copy-outline /></n-icon>
          </template>
        </n-button>
...
import {
  NCode, NCollapse, NCollapseItem, NDatePicker, useMessage, NInput,
  NSpace, NStatistic, NNumberAnimation, NButton, NIcon } from "naive-ui"
