...
func getConnectVariableHeader(r io.Reader) (hdr ConnectVariableHeader, len int, err error) {
	...
	connectRecieveMaximum := make([]byte, 2)
	n, err = r.Read(connectRecieveMaximum)
	len += n
	if err != nil {
		return hdr, len, errors.New("Could not read recieve maximum byte")
	}
	if n > 0 {
		hdr.ConnectProperties.RecieveMaximumValue = int(binary.BigEndian.Uint16(connectRecieveMaximum))
	}
	return
}
